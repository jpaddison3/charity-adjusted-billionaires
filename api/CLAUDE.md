...
- The repository pins Node 24.x in `package.json` and `.nvmrc`. Vercel uses `engines.node` from `package.json` for the function runtime, overriding the dashboard's Node version setting.
