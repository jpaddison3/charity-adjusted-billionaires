...
- Verify with: `npm run test:run` (fast, ~6s), `npm run lint` (~2s), `npm run build`. CI (`.github/workflows/ci.yml`) runs lint → skills sync check → coverage-gated tests (50% floors via `npm run test:coverage`) → build on pushes to main and PRs; tests and build each generate their data. E2e (`npm run test:e2e`) is chromium-only by design.
